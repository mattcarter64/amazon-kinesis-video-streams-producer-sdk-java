...
public final class RtspDemoAppMain {
	...
	// Need to get key frame configured properly so the output can be decoded. h264
	// files can be decoded using gstreamer plugin
	// gst-launch-1.0 rtspsrc location="YourRtspUri" short-header=TRUE protocols=tcp
	// ! rtph264depay ! decodebin ! videorate ! videoscale ! vtenc_h264_hw
	// allow-frame-reordering=FALSE max-keyframe-interval=25 bitrate=1024
	// realtime=TRUE !
	// video/x-h264,stream-format=avc,alignment=au,profile=baseline,width=640,height=480,framerate=1/25
	// ! multifilesink location=./frame-%03d.h264 index=1
	...
	private static MediaSource createCameraMediaSource() {
		...
		// byte[] cpd = { 0x01, 0x42, 0x00, 0x20, (byte) 0xff, (byte) 0xe1, 0x00, 0x23,
		// 0x27, 0x42, 0x00, 0x20,
		// (byte) 0x89, (byte) 0x8b, 0x60, 0x28, 0x02, (byte) 0xdd, (byte) 0x80, (byte)
		// 0x9e, 0x00, 0x00, 0x4e,
		// 0x20, 0x00, 0x0f, 0x42, 0x41, (byte) 0xc0, (byte) 0xc0, 0x01, 0x77, 0x00,
		// 0x00, 0x5d, (byte) 0xc1, 0x7b,
		// (byte) 0xdf, 0x07, (byte) 0xc2, 0x21, 0x1b, (byte) 0x80, 0x01, 0x00, 0x04,
		// 0x28, (byte) 0xce, 0x1f,
		// 0x20 };

		final CameraMediaSourceConfiguration configuration = new CameraMediaSourceConfiguration.Builder()
				.withFrameRate(FPS_25).withRetentionPeriodInHours(1).withCameraId("cam1")
		...
		// final CameraMediaSource mediaSource = new CameraMediaSource("rtsp-stream",
		// camera);
	...
	private static MediaSource createImageFileMediaSource() {
		final ImageFileMediaSourceConfiguration configuration = new ImageFileMediaSourceConfiguration.Builder()
				.fps(FPS_25).dir(IMAGE_DIR).filenameFormat(IMAGE_FILENAME_FORMAT).startFileIndex(START_FILE_INDEX)
				.endFileIndex(END_FILE_INDEX).build();
	...
	private static MediaSource createFileMediaSource() {
		final AudioVideoFileMediaSourceConfiguration configuration = new AudioVideoFileMediaSourceConfiguration.AudioVideoBuilder()
				.withDir(FRAME_DIR).withRetentionPeriodInHours(RETENTION_ONE_HOUR)
				.withAbsoluteTimecode(ABSOLUTE_TIMECODES).withTrackInfoList(DemoTrackInfos.createTrackInfoList())
				.build();
